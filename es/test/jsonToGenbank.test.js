...
chai.should();
describe("genbank exporter/parser conversion", () => {
  ...
  it(`
    ...
    with a note of pragma: ['Teselagen_Part'] on it, and by default convert those features back into parts  `, () => {
  ...
  it("should parse notes that come in as a JSON stringified object correctly", () => {
    // const breakingJSON = require('./testData/json/breakingJSON_stringified')
  ...
  it("can interconvert between our parser and our exporter with a malformed genbank", done => {
    ...
    parseGenbank(string, result => {
      ...
      parseGenbank(exportedGenbankString, result => {
        result.should.be.an("array");
        result[0].success.should.be.true;
        result[0].parsedSequence.features.should.be.length(13);
  ...
  it("parses and converts pj5_00001 (aka testGenbankFile.gb) correctly (handling joined feature spans correctly also)", done => {
    const string = fs.readFileSync(
  ...
  it("parses and converts a genbank with just feature start locations correctly", done => {
    const string = fs.readFileSync(
  ...
  it("handles features in an array or a keyed object", done => {
  ...
  it("should export warnings, assemblyPieces, and lineageAnnotations, as features with pragmas, preserving color and label color", done => {
    const exportedGenbankString = jsonToGenbank({
  ...
  it("should export primers as features with type set as primer", done => {
    const exportedGenbankString = jsonToGenbank({
      name: "testing_primer_export",
      sequence: "ATGCATTGAGGACCTAACCATATCTAA",
      type: "DNA",
  ...
  it("handles inclusive1BasedStart and inclusive1BasedEnd options", done => {
    const exportedGenbankString = jsonToGenbank(
      {
        sequence: "gagagagagga",
        features: {
  ...
  it("gives genbank that is linear when circular is falsy", done => {
    const exportedGenbankString = jsonToGenbank({
      sequence: "gagagagagga",
      circular: false
    });
  ...
  it('gives genbank that is linear when sequence.circular="0"', done => {
    const exportedGenbankString = jsonToGenbank({
      sequence: "gagagagagga",
      circular: "0"
    });
  ...
  it("handles reformatSeqName=false option", done => {
    const name = "$%^@#";
    const exportedGenbankString = jsonToGenbank(
      {
        sequence: "gagagagagga",
  ...
  // it('handles reformatSeqName=true (this is on by default) option', done => {
  //     const name = '$%^@#'
  ...
  //     },{reformatSeqName: false});
  // });
  it("does not reformat a name with parens in it", done => {
    const name = "aaa(aaa)";
    const exportedGenbankString = jsonToGenbank(
      {
        sequence: "gagagagagga",
  ...
  it("provides a default name if none is provided", done => {
    const exportedGenbankString = jsonToGenbank(
      {
        sequence: "gagagagagga"
      },
    ...
    parseGenbank(exportedGenbankString, result => {
      ...
      parseGenbank(jsonToGenbank({ sequence: "gagagagagga" }), result => {
        result[0].parsedSequence.name.should.equal("Untitled_Sequence");
        done();
      });
    });
  });
  it("adds a comment with the words teselagen_unique_id: XXXX if given a .teselagen_unique_id property", done => {
    const exportedGenbankString = jsonToGenbank({
      sequence: "gagagagagga",
  ...
  it("adds a comment for the library field if the sequence has one", done => {
    const exportedGenbankString = jsonToGenbank({
      sequence: "gagagagagga",
  ...
  it("adds a comment for the description if the sequence has one", done => {
    const exportedGenbankString = jsonToGenbank({
      sequence: "gagagagagga",
  ...
  it("handles comments parsing and formatting", done => {
    const exportedGenbankString = jsonToGenbank({
      sequence: "gagagagagga",
